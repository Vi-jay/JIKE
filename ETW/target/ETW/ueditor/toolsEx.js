...
UE.registerUI('myblockquote', function (editor, uiName) {
    ...
    var btn = new UE.ui.Button({
        name: uiName,
        title: '整句引用',
        cssRules: 'background-image:url(' + sy.basePath + 'ueditor/themes/default/images/yingyong.png)!important;\n    background-size: 100% 100%;',
        onclick: function onclick() {
            editor.execCommand(uiName);
        }
    });
...
UE.registerUI('myblockquote1', function (editor, uiName) {
    ...
    var btn = new UE.ui.Button({
        name: uiName,
        title: '浮动引用',
        cssRules: 'background-image:url(' + sy.basePath + 'ueditor/themes/default/images/float_left.png)!important;\n    background-size: 100% 100%;',
        onclick: function onclick() {
            editor.execCommand(uiName);
        }
    });
...
var toggleFlag = { removeFlag: false, showBorderFlag: false };
//自定义引用样式例子
UE.registerUI('myblockquote2', function (editor, uiName) {
    ...
    var btn = new UE.ui.Button({
        name: uiName,
        title: '清除单元格边框样式',
        cssRules: 'background-image:url(' + sy.basePath + 'ueditor/themes/default/images/table_dashed.png)!important;\n    background-size: 100% 100%;',
        onclick: function onclick() {
    ...
    editor.addListener('selectionchange', function () {
        ...
        if (toggleFlag.removeFlag) {
            var selectDom = editor.selection.getRange().endContainer;
            if (+selectDom.nodeType === 1) {
                var parentDom = getParentDom(selectDom, "TABLE");
...
UE.registerUI('myblockquote3', function (editor, uiName) {
    ...
    var btn = new UE.ui.Button({
        name: uiName,
        title: '显示单元格边框样式',
        cssRules: 'background-image:url(' + sy.basePath + 'ueditor/themes/default/images/table.png)!important;\n    background-size: 100% 100%;',
        onclick: function onclick() {
