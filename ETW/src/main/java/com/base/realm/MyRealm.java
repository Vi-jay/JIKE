...
public class MyRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        UserEx user = userService.getUserByUser(userModel);
        if (null != user) {
            ...
            if (null != user.getRoles() && user.getRoles().size() > 0) {
                ...
                for (Role role : user.getRoles()) {
                    ...
                    RoleEx roleEx = roleService.getRoleByRole(role);
                    if (null != roleEx.getPermissions() && roleEx.getPermissions().size() > 0) {
                        //获取权限
                        for (Permissions pmss : roleEx.getPermissions()) {
...
//        }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
        ...
        userModel.setPassword(new String(token.getPassword()));
        User user = userService.getUserByUser(userModel);
        if (null != user) {
